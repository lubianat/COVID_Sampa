...
dados_sp_selecionada <- resdados_sp %>% 
  ...
  setNames(c("Data", "Registros de caso cumulativos", "Registros de óbito cumulativos", "Dia de contagem")) %>% 
  as.data.frame() # Cause diff doesn't work on tibbles

dados_sp_selecionada["Registros de caso por dia"] <- pegar_valor_por_dia(dados_sp_selecionada[, "Registros de caso cumulativos"]) 
dados_sp_selecionada["Registros de óbito por dia"] <- pegar_valor_por_dia(dados_sp_selecionada[, "Registros de óbito cumulativos"]) 
```
...
dados_sp_selecionada_long <- pivot_longer(dados_sp_selecionada,
  cols = c("Registros de caso cumulativos", "Registros de óbito cumulativos", "Registros de caso por dia", "Registros de óbito por dia"),
  names_to = "Tipo",
  values_to = "Contagem"
)
```
...
so_casos <- dados_sp_selecionada_long %>% 
  filter(`Tipo` %in% c("Registros de caso cumulativos", "Registros de caso por dia"))

ultima_data <- dados_sp_selecionada_long[["Data"]][1]


rotulo_eixo_x <- paste0("Data", " (dados até ", ultima_data, ")")
...
so_casos_por_dia <- dados_sp_selecionada_long %>%
  filter(`Tipo` %in% c("Registros de caso por dia"))
...
so_obitos <- dados_sp_selecionada_long %>% 
  filter(`Tipo` %in% c("Registros de óbito cumulativos", "Registros de óbito por dia"))
...
so_obitos_diarios <- dados_sp_selecionada_long %>% 
  filter(`Tipo` %in% c("Registros de óbito por dia"))
